...
from .u115_direct import Direct115Client
...
class Panlink115(_PluginBase):
    ...
    _u115_client: Optional[Direct115Client] = None
    ...
    def init_plugin(self, config: dict = None):
        ...
        self._u115_cookie = str(config.get("u115_cookie") or "").strip()
        self._restore_state()
        self._client = self._build_client()
        self._u115_client = self._build_u115_client()
        logger.info(
            "Panlink115 plugin initialized: has_u115_cookie=%s mp_u115_ready=%s",
            bool(self._u115_cookie),
            self._get_u115_auth_info().get("ready"),
        )
    ...
    def api_state(self) -> Dict[str, Any]:
        auth_info = self._get_u115_auth_info()
        return {
            "success": True,
            ...
            "u115_auth_mode": auth_info.get("mode"),
            "u115_auth_label": auth_info.get("label"),
            "u115_cookie_configured": auth_info.get("has_cookie"),
            "mp_u115_configured": auth_info.get("configured"),
            "mp_u115_ready": auth_info.get("ready"),
            "workflow_mode": self._get_workflow_mode(),
            "workflow_label": self._get_workflow_label(),
            "direct_target_hint": auth_info.get("target_hint"),
        }
    ...
    def api_queue_115(self, title: str = "", url: str = "", password: str = "", source: str = "", vod_id: str = "", vod_name: str = "", type_name: str = "", category_group: str = "", category_name: str = "") -> Dict[str, Any]:
        ...
        if not group or not name:
            ...
            return {"success": False, "message": self._last_message, "queue": list(self._queued_115)}
        try:
            share_url, share_password = Direct115Client.normalize_share_link(raw_url, password)
            target = self._resolve_u115_target(group, name)
            client = self._build_u115_client()
            logger.info(
                "Panlink115 submit via direct 115 receive: target=%s cid=%s workflow=%s",
                target.get("path"),
                target.get("cid"),
                workflow_mode,
            )
            result = client.receive_share(url=share_url, target_cid=str(target.get("cid") or ""), password=share_password)
        except Exception as err:
            logger.error(
                "Panlink115 submit to direct 115 failed: has_u115_cookie=%s mp_u115_ready=%s error=%s",
                bool(self._u115_cookie),
                self._get_u115_auth_info().get("ready"),
                err,
            )
        ...
        item = {
            ...
            "submit_path": target.get("path") or "",
            "organize_path": target.get("path") or "",
            "submit_method": "direct_115",
            "workflow_mode": workflow_mode,
            "workflow_label": self._get_workflow_label(),
            "created_name": "",
            "created_path": "",
            "already_exists": already_exists,
            "auth_label": "115 网页 Cookie",
            "target_cid": result.get("target_cid") or target.get("cid") or "",
    ...
    def _build_u115_client(self) -> Direct115Client:
        if not self._u115_client:
            self._u115_client = Direct115Client(cookie=self._u115_cookie, timeout=self._timeout)
        else:
            self._u115_client.update_config(cookie=self._u115_cookie, timeout=self._timeout)
        return self._u115_client

    def _get_mp_u115_oper(self):
        try:
            from app.modules.filemanager.storages.u115 import U115Pan
        except Exception as err:
            raise RuntimeError(f"无法导入 MoviePilot u115 存储模块：{err}") from err
        return U115Pan()

    def _get_u115_auth_info(self) -> Dict[str, Any]:
        info: Dict[str, Any] = {
            "mode": "direct_115",
            "label": "115 网页 Cookie",
            "has_cookie": bool(self._u115_cookie),
            "configured": False,
            "ready": False,
            "target_hint": "",
        }
        try:
            u115 = self._get_mp_u115_oper()
            conf = u115.get_conf() or {}
            access_token = u115.access_token
            info["configured"] = bool(conf.get("refresh_token") or conf.get("access_token"))
            info["ready"] = bool(access_token)
        except Exception as err:
            logger.warning("Panlink115 failed to read MP u115 auth state: %s", err)
        return info

    def _resolve_u115_target(self, category_group: str, category_name: str) -> Dict[str, Any]:
        mp_target = self._resolve_mp_target_directory(category_group, category_name)
        if not mp_target:
            raise RuntimeError(f"未找到分类“{category_group} / {category_name}”对应的 MoviePilot 目录")

        storage = str(mp_target.get("library_storage") or "").strip().lower()
        target_path = self._normalize_fs_path(mp_target.get("target_path") or mp_target.get("library_path"))
        if storage != "u115":
            raise RuntimeError(
                f"当前分类对应的 MoviePilot 存储不是 u115，而是“{mp_target.get('library_storage') or 'local'}”，无法直连提交到 115"
            )
        if not target_path:
            raise RuntimeError("MoviePilot u115 目标目录为空，无法提交")

        folder = self._get_mp_u115_oper().get_folder(Path(target_path))
        if not folder or not folder.fileid:
            raise RuntimeError(f"无法获取或创建 115 目标目录：{target_path}")
        return {
            "storage": "u115",
            "path": self._normalize_fs_path(folder.path or target_path),
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            ...
            {"path": "/search", "endpoint": self.api_search, "methods": ["GET"], "summary": "盘链搜索", "auth": "bear"},
            {"path": "/load_links", "endpoint": self.api_load_links, "methods": ["GET"], "summary": "加载资源详情与分享链接", "auth": "bear"},
            {"path": "/queue_115", "endpoint": self.api_queue_115, "methods": ["GET"], "summary": "直连提交到 115", "auth": "bear"},
            {"path": "/clear_queue", "endpoint": self.api_clear_queue, "methods": ["GET"], "summary": "清空任务队列", "auth": "bear"},
        ]
                    ...
                    "content": [
                        {
                            ...
                            "content": [
                                ...
                                },
                                {
                                    "component": "VCol",
                                    "props": {"cols": 12, "md": 6},
                                    "content": [{"component": "VSwitch", "props": {"model": "only_show_115", "label": "仅显示 115 资源"}}],
                                },
                            ],
                        },
                        {
                            ...
                            "content": [
                                {
                                    "component": "VCol",
                                    "props": {"cols": 12, "md": 6},
                                    "content": [{"component": "VTextField", "props": {"model": "username", "label": "盘链账号"}}],
                                },
                                {
                                    "component": "VCol",
                                    "props": {"cols": 12, "md": 6},
                                    "content": [{"component": "VTextField", "props": {"model": "password", "label": "盘链密码", "type": "password"}}],
                                },
                            ],
                        },
                        {
                            ...
                            "content": [
                                {
                                    "component": "VCol",
                                    "props": {"cols": 12, "md": 6},
                                    "content": [{"component": "VTextField", "props": {"model": "timeout", "label": "请求超时秒数", "type": "number"}}],
                                },
                                {
                                    "component": "VCol",
                                    "props": {"cols": 12, "md": 6},
                                    "content": [{"component": "VTextField", "props": {"model": "max_results", "label": "搜索结果数量", "type": "number"}}],
                                },
                            ],
                        },
                        {
                        ...
                        },
                        {
                            ...
                            "props": {
                                "type": "info",
                                "variant": "tonal",
                                "text": "插件已切换为直连 115 模式，不再依赖 CD2。目标目录复用 MoviePilot 的 u115 存储；分享转存通过 115 网页 Cookie 调用官方网页接口。",
                            },
                        },
                    ],
                }
    ...
    def api_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "u115_auth_mode": auth_info.get("mode"),
            "u115_auth_label": auth_info.get("label"),
            "u115_cookie_configured": bool(auth_info.get("has_cookie")),
            "mp_u115_configured": bool(auth_info.get("configured")),
            "mp_u115_ready": bool(auth_info.get("ready")),
            "workflow_mode": self._get_workflow_mode(),
            "workflow_label": self._get_workflow_label(),
            "direct_target_hint": auth_info.get("target_hint") or "",
        }
    ...
    def api_queue_115(
        ...
        try:
            share_url, share_password = Direct115Client.normalize_share_link(raw_url, password)
            target = self._resolve_u115_target(group, name)
            result = self._build_u115_client().receive_share(
                url=share_url,
            ...
            logger.error(
                "Panlink115 direct 115 submit failed: has_u115_cookie=%s mp_u115_ready=%s error=%s",
                bool(self._u115_cookie),
                self._get_u115_auth_info().get("ready"),
                err,
            )
            self._last_message = f"提交 115 分享转存失败：{err}"
            self._persist_state()
            return {"success": False, "message": self._last_message, "queue": list(self._queued_115)}
        ...
        item = {
            ...
            "target_cid": result.get("target_cid") or target.get("cid") or "",
            "queued_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "organize_status": "直连 115 分类直投",
            "status": "115 分享转存任务已存在" if already_exists else "已提交 115 分享转存",
        }
        if not any(
        ...
        self._persist_state()
        logger.info(
            "Panlink115 submitted 115 shared-link task via direct 115: %s -> %s (already_exists=%s)",
            vod_name,
            item["target_path"],
            already_exists,
        )
...
class Panlink115(_Panlink115AbstractBase):
    ...
    def api_diagnose_cd2(self, category_group: str = "", category_name: str = "") -> Dict[str, Any]:
        self._last_message = "CD2 提交链路已移除，请直接使用 115 网页 Cookie + MoviePilot 的 u115 存储。"
        self._persist_state()
        return {
            "success": False,
            "message": self._last_message,
