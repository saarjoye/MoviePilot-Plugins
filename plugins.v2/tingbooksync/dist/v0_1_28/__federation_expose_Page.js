...
const { computed, defineComponent, h, onBeforeUnmount, onMounted, reactive, ref, resolveComponent } = await importShared("vue");
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedRecordCount = computed(() => selectedRecordDirs.value.length);
    const allFilteredRecordsSelected = computed(() => {
      const dirs = filteredRecords.value.map((item) => item.bookDir).filter(Boolean);
      return !!dirs.length && dirs.every((bookDir) => selectedRecordDirs.value.includes(bookDir));
    });
    ...
    async function fetchRecords(options = {}) {
      if (!options.silent) recordLoading.value = true;
      try {
        const payload = await request("plugin/TingBookSync/records?limit=300");
        records.value = Array.isArray(payload?.items) ? payload.items : [];
        const knownDirs = new Set(records.value.map((item) => item.bookDir).filter(Boolean));
        selectedRecordDirs.value = selectedRecordDirs.value.filter((bookDir) => knownDirs.has(bookDir));
      } catch (err) {
        error.value = err?.message || "读取整理记录失败";
      } finally {
        if (!options.silent) recordLoading.value = false;
      }
    }

    function isRecordSelected(item) {
      return !!item?.bookDir && selectedRecordDirs.value.includes(item.bookDir);
    }

    function setRecordSelected(item, selected) {
      if (!item?.bookDir) return;
      const next = selectedRecordDirs.value.filter((bookDir) => bookDir !== item.bookDir);
      if (selected) next.push(item.bookDir);
      selectedRecordDirs.value = next;
    }

    function toggleFilteredRecords(selected) {
      const visibleDirs = filteredRecords.value.map((item) => item.bookDir).filter(Boolean);
      if (selected) {
        selectedRecordDirs.value = Array.from(new Set([...selectedRecordDirs.value, ...visibleDirs]));
      } else {
        const visibleSet = new Set(visibleDirs);
        selectedRecordDirs.value = selectedRecordDirs.value.filter((bookDir) => !visibleSet.has(bookDir));
      }
    }
    ...
    async function resetSelectedRecords() {
      const bookDirs = selectedRecordDirs.value.slice();
      if (!bookDirs.length) return;
      if (!window.confirm(`确认覆盖重新生成选中的 ${bookDirs.length} 本 STRM？\n会优先使用本地 pickcode；没有时从 115 已有文件补齐 pickcode，不会主动重新上传。`)) return;
      recordLoading.value = true;
      activeRecordAction.value = "batch-strm";
      const refreshTimer = window.setInterval(() => {
        fetchRecords({ silent: true });
        fetchLogs({ silent: true });
      }, 2000);
      message.value = `正在批量覆盖重新生成 ${bookDirs.length} 本 STRM，请等待...`;
      try {
        const payload = await request(`plugin/TingBookSync/records/reset-batch${buildQuery({ bookDirs: JSON.stringify(bookDirs), mode: "strm" })}`, { method: "POST" });
        message.value = payload?.message || "批量覆盖重新生成 STRM 已完成";
        if (!payload?.failed) selectedRecordDirs.value = [];
        await fetchRecords();
        await fetchLogs();
      } catch (err) {
        error.value = err?.message || "批量覆盖重新生成 STRM 失败";
      } finally {
        window.clearInterval(refreshTimer);
        activeRecordAction.value = "";
        recordLoading.value = false;
    ...
    function renderRecord(item, index) {
      ...
      return h(c("VCard"), { key: `${item.bookDir || title}-${index}`, variant: "outlined", rounded: "lg", class: "mb-3" }, () => h(c("VCardText"), null, () => [
        h("div", { class: "d-flex align-start justify-space-between ga-3 flex-wrap" }, [
          h(c("VCheckboxBtn"), { modelValue: isRecordSelected(item), disabled: recordLoading.value || !item.bookDir, "onUpdate:modelValue": (value) => setRecordSelected(item, !!value), "aria-label": `选择 ${title}` }),
    ...
    function renderRecordsPanel() {
      return h(c("VCard"), { variant: "outlined", rounded: "lg" }, () => [
        ...
        h(c("VCardText"), null, () => [
          ...
          filteredRecords.value.length ? h("div", { class: "d-flex align-center justify-space-between ga-2 flex-wrap mb-3" }, [
            h(c("VCheckboxBtn"), { modelValue: allFilteredRecordsSelected.value, disabled: recordLoading.value, "onUpdate:modelValue": (value) => toggleFilteredRecords(!!value), label: `全选当前筛选结果（已选 ${selectedRecordCount.value} 本）` }),
            h(c("VBtn"), { color: "primary", variant: "flat", prependIcon: "mdi-link-variant-plus", loading: recordLoading.value && activeRecordAction.value === "batch-strm", disabled: recordLoading.value || !selectedRecordCount.value, onClick: resetSelectedRecords }, () => selectedRecordCount.value ? `批量覆盖生成 STRM（${selectedRecordCount.value}）` : "批量覆盖生成 STRM"),
          ]) : null,
