...
export default defineComponent({
  ...
  setup() {
    ...
    async function resetRecord(item) {
      if (!item?.bookDir) return;
      if (!window.confirm(`确认重新处理《${item.title || item.bookName || item.bookDir}》？\n会删除该书旧同步状态和安全范围内的旧 STRM 目录，下次扫描会重新上传 115 并生成 STRM。`)) return;
      recordLoading.value = true;
      try {
        const payload = await request(`plugin/TingBookSync/records/reset${buildQuery({ bookDir: item.bookDir })}`, { method: "POST" });
        message.value = payload?.message || "已重置整理记录";
        await fetchRecords();
        await fetchLogs();
      } catch (err) {
    ...
    function renderRecord(item, index) {
      ...
      return h(c("VCard"), { key: `${item.bookDir || title}-${index}`, variant: "outlined", rounded: "lg", class: "mb-3" }, () => h(c("VCardText"), null, () => [
        ...
        h("div", { class: "d-flex align-center justify-space-between ga-2 flex-wrap mt-3" }, [
          ...
          h(c("VBtn"), { color: "warning", variant: "tonal", size: "small", prependIcon: "mdi-restore-alert", loading: recordLoading.value, onClick: () => resetRecord(item) }, () => "重新处理"),
        ]),
      ]));
    }
      ...
      return h(c("VCard"), { variant: "outlined", rounded: "lg" }, () => [
        h(c("VCardTitle"), { class: "d-flex align-center justify-space-between flex-wrap ga-2" }, () => [
          h("div", [
            ...
            h("div", { class: "text-caption text-medium-emphasis" }, "按书记录上传、STRM 和 ADS 处理结果，出问题时可单本重新处理。"),
          ]),
          h("div", { class: "d-flex ga-2 flex-wrap" }, [
            h(c("VTextField"), {
