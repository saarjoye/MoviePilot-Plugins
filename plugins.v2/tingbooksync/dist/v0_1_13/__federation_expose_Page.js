      ...
      h(c("VRow"), null, () => [
        ...
        h(c("VCol"), { cols: "12", lg: "4" }, () => h("div", { class: "d-flex flex-column ga-4" }, [
          ...
          h(c("VCard"), { variant: "outlined", rounded: "lg" }, () => [
            ...
            h(c("VCardText"), null, () => [
              h("div", { class: "d-flex ga-2 flex-wrap mb-3" }, LOG_TABS.map((tab) => h(c("VChip"), {
                key: tab.key,
              ...
              filteredLogs.value.length ? h(c("VList"), { density: "compact", lines: "two", style: "max-height: 380px; overflow-y: auto;" }, () => filteredLogs.value.map((item, index) => h(c("VListItem"), {
                key: `${item.time}-${index}`,
                title: `${item.stage || "runtime"} - ${item.message || ""}`,
                subtitle: item.time || "",
              }, {
                prepend: () => h(c("VIcon"), { color: levelColor(item.level), icon: item.level === "error" ? "mdi-alert-circle-outline" : "mdi-information-outline" }),
